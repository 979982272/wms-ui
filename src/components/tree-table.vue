  ...
  <div class="tree-table">
    <div class="toolbar">
      <el-button v-for="item in toolbar" size="mini" :type="item.type" :key="item.name"
                 @click="itemMethod(item.click)" v-if="toolbar" :disabled="item.disabled">
        {{item.text}}
      </el-button>
      ...
      <div class="tree-head">
        <table>
          <tr style="padding-left: 30px">
            ...
            <td v-for="(column,index) in gridColumns"
                :key="column.name" style="padding-left: 30px" :class="'td'+(index+1)">
              {{ column.title}}
            </td>
          </tr>
        </table>
...
export default {
  name: 'treeTable',
  props: ['list', 'gridColumns', 'command', 'toolbar'],
  data () {
    return {
      isDesc: false,
      treeDataSource: []
  ...
  methods: {
    ...
    itemMethod (method, item) {
      this.$emit(method, item)
    },
    // 刷新
    refresh () {
      this.$emit('callGetInfo', 1, {})
    }
  },
  components: {
