...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        {label: '角色名称', prop: 'roleName', required: true, type: 'input'},
  ...
  methods: {
    getModelDataInfo (id) {
      this.$post('/base/role/' + id + '/getDataInfoById').then((res) => {
    ...
    saveEditInfo (formName) {
      this.$refs.editform.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$post('/base/role/' + this.op, data, {headers: {'Content-Type': 'application/json'}})
            .then((res) => {
              if (res.status) {
