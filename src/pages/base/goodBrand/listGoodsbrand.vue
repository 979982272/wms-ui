<template>
  <div>
    ...
    <el-dialog title="编辑产品品牌" :visible.sync="dialogVisible" @close="closeDialog">
...
export default {
  ...
  data () {
    return {
      ...
      // 列信息
      gridColumns: [
        {name: 'chnName', title: '中文名称', width: 0},
        {name: 'engName', title: '英文名称', width: 0},
        {name: 'remark', title: '备注', width: 0}
      ],
      // 列数据
      gridData: {},
      // 列按钮信息
  ...
  methods: {
    callGetInfo (page, filter) {
      ...
      param['filter'] = filter
      this.$post('/base/goodsBrand/loadData', param, {headers: {'Content-Type': 'application/json'}})
        .then((res) => {
          this.gridData = res
        })
    },
    editInfo (item) {
      ...
      this.$router.push({path: '/base/goodBrand/editGoodsBrand', query: {id: item.id}})
    },
    create (checkedInfo) {
      this.dialogVisible = true
      this.$router.push('/base/goodBrand/editGoodsBrand')
    },
    deleteInfo (item) {
      this.$delete('/base/goodsBrand/' + item.id + '/deleteById').then((res) => {
        if (res.status) {
