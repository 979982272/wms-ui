...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        {label: '中文名称', prop: 'chnName', required: true, type: 'input'},
        {label: '英文名称', prop: 'engName', required: true, type: 'input'},
  ...
  methods: {
    getModelDataInfo (id) {
      this.$post('/base/goodsBrand/' + id + '/getDataInfoById').then((res) => {
    ...
    saveEditInfo (formName) {
      this.$refs.editform.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$post('/base/goodsBrand/' + this.op, data, {headers: {'Content-Type': 'application/json'}})
            .then((res) => {
              if (res.status) {
