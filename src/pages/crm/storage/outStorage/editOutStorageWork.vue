  ...
  data () {
    return {
      ...
      columns: [
        ...
        {label: '客户名称', prop: 'customerName', type: 'input', disabled: true},
      ...
      gridColumns: [
        ...
        {name: 'deliveryAmount', title: '出库数量', search: true},
        {
          name: 'waitDeliveryAmount',
          title: '待出库数量',
          search: true,
          template: {type: 'inputNumber', prop: 'waitDeliveryAmount'}
        },
      ...
      gridHeight: 510,
      command: [
        {name: 'opOutStorage', text: '<span class="el-icon-upload2"></span>', tooltip: '出库', click: 'outStorage'}
      ]
    }
  },
  methods: {
    getModelDataInfo (id) {
      this.$post('/crm/outStorage/outStorageWork/' + id + '/getDataInfoById').then((res) => {
        this.modelData = res.other.modelData
        this.gridData['data'] = res.other.modelData.outStorageWorkParts
        this.gridData['data'].forEach(function (item) {
          item.waitDeliveryAmount = item.planAmount - item.deliveryAmount
        })
        this.op = 'update'
      })
    },
    ...
    onDataBound () {
      if (Object.keys(this.gridData).length > 0) {
        this.gridData.data.forEach(function (item) {
          let opOutStorage = document.getElementById('opOutStorage' + item.id)
          if (parseInt(item.deliveryAmount) === parseInt(item.planAmount)) {
            if (opOutStorage !== null) {
              opOutStorage.remove()
            }
          }
        })
    ...
    outStorage (item) {
      this.$post('/crm/outStorage/outStorageWorkPart/outStorageByPart?id=' + item.id + '&deliveryAmount=' + item.waitDeliveryAmount, {}, {headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}})
        .then((res) => {
          if (res.status) {
